        ...
        public function undressUpdate ($dressedUpdate, $botName, $cmAuthInfo, $textToPut = null, $userIdToPut = null)
        {
            ...
            elseif (isset($update->message)) {
                ...
                if (isset($update->message->photo)) {
                    ...
                    $add2resources(                                       InteractionResource::createFromFileId( InteractionResource::TYPE_IMAGE,     $this->type, $cmAuthInfo, $chosen->file_id,                      $extract($chosen,                      'width,height,mime_type,format=jpg')          ));
                }
                if (isset($update->message->audio))      { $add2resources(InteractionResource::createFromFileId( InteractionResource::TYPE_AUDIO,     $this->type, $cmAuthInfo, $update->message->audio->file_id,      $extract($update->message->audio,      'duration,mime_type,format=m4a')              )); }
                if (isset($update->message->voice))      { $add2resources(InteractionResource::createFromFileId( InteractionResource::TYPE_VOICE,     $this->type, $cmAuthInfo, $update->message->voice->file_id,      $extract($update->message->voice,      'duration,mime_type,format=oga')              )); }
                if (isset($update->message->video))      { $add2resources(InteractionResource::createFromFileId( InteractionResource::TYPE_VIDEO,     $this->type, $cmAuthInfo, $update->message->video->file_id,      $extract($update->message->video,      'width,height,duration,mime_type,format=mp4') )); }
                if (isset($update->message->video_note)) { $add2resources(InteractionResource::createFromFileId( InteractionResource::TYPE_VIDEONOTE, $this->type, $cmAuthInfo, $update->message->video_note->file_id, $extract($update->message->video_note, 'length,duration,mime_type,format=mp4')       )); }
                if (isset($update->message->document))   { $add2resources(InteractionResource::createFromFileId( InteractionResource::TYPE_DOCUMENT,  $this->type, $cmAuthInfo, $update->message->document->file_id,   $extract($update->message->document,   'mime_type,format')                           )); }
